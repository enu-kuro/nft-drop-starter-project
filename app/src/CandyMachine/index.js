...
const CandyMachine = ({ walletAddress }) => {
  ...
  React.useEffect(() => {
    const getMitedNfts = async () => {
      ...
      for (const nft of findMany) {
        ...
        let imgUri = await response.json();
        imgUris.push(imgUri);
      }
      setMints(imgUris);
  ...
  const renderMintedItems = (mints) => {
    return (
      <div className="gif-container">
        {mints.map((m) => {
          console.log(m);
          return <img className="gif-item" key={m.image} src={m.image}></img>;
        })}
      </div>
    );
  };
  ...
  const currentDate = new Date();
  const dropDate =
    candyMachine && new Date(candyMachine.state.goLiveData * 1000);
  ...
  return (
    candyMachine && (
      <div className="machine-container">
        ...
        {currentDate < dropDate && renderDropTimer()}
        ...
        {mints.length > 0 && renderMintedItems(mints)}
